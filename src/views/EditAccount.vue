    ...
    <v-col align="center" padding="0">
      <v-form class="form" ref="form" lazy-validation>
        ...
        <v-text-field
          ...
          :rules="enable && confirmPassword ? passwordRules : []"
        ...
        <v-btn class="save_button" @click="save">Save</v-btn>
      </v-form>
    </v-col>
  </div>
</template>
...
export default {
  data: () => ({
    ...
    passwordRules: [
      (v) =>
        (v && v.length >= 8) ||
        "Password must have 8+ characters, one uppercase character, one number and one special character.",
      (v) =>
        /(?=.*[A-Z])/.test(v) ||
        "Password must have 8+ characters, one uppercase character, one number and one special character.",
      (v) =>
  ...
  async created() {
    ...
    user.providerData.forEach((profile) => {
      if (profile.providerId == "google.com") this.enabled = false;
    });
  ...
  methods: {
    ...
    async save() {
      ...
      if (valid.valid) {
        ...
        const photoURL =
          this.photoURL != "" ? this.photoURL : this.photoURL_old;
        let email,
          password = false;
        if (this.currentPassword) {
          let authed = await reAuth(this.currentPassword);
          if (!authed) return (this.errorMessage = "Incorrect Password");
        ...
        if (editted) this.$router.push({ name: "Profile" });
      }
    },
  },
};
